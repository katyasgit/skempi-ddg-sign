...
Качество тут заведомо скромное, по условию им можно пренебречь. Если бы цель
была именно в качестве, я бы в первую очередь добавила структурные признаки —
SKEMPI даёт PDB-структуры, из них можно посчитать ΔSASA, контакты остатка,
плотность упаковки. Ещё напрашиваются эмбеддинги белковых языковых моделей
(ESM-2) вместо ручных признаков, регрессия величины ddG вместо классификации
знака (знак потом получается порогом), и нейтральная зона |ddG| < 0.5
ккал/моль — такие значения сравнимы с ошибкой эксперимента, честнее их не
учить. Гиперпараметры я специально не подбирала, модели почти «из коробки».
